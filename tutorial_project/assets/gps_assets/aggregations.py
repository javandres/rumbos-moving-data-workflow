...
def make_aggregations_by_code(code):
    ...
    def asset_template():
        ...
        aggregator_slow = mpd.TrajectoryCollectionAggregator(
            traj_collection_slow,
            max_distance=100,
            min_distance=10,
            min_stop_duration=timedelta(minutes=5),
        )
        ...
        if flows_slow.empty:
            flows_slow = gpd.GeoDataFrame(
                columns=["id", "length", "weight", "geometry"], geometry="geometry"
            )
        else:
        ...
        try:
            gdf_motorized = gpd.GeoDataFrame.from_postgis(
                sql_motorized, conn, geom_col="geometry"
            )
            gdf_motorized = gdf_motorized.set_index("tracked_at")
            gdf_motorized["time"] = gdf_motorized.index

            traj_collection_motorized = mpd.TrajectoryCollection(
                gdf_motorized, "track_id", t="time"
            )
            aggregator_motorized = mpd.TrajectoryCollectionAggregator(
                traj_collection_motorized,
                max_distance=100,
                min_distance=10,
                min_stop_duration=timedelta(minutes=5),
            )
            ...
            if flows_motorized.empty:
                flows_motorized = gpd.GeoDataFrame(
                    columns=["id", "length", "weight", "geometry"], geometry="geometry"
                )
            else:
            ...
            clusters_motorized = gpd.GeoDataFrame(
                columns=["id", "n", "geometry"], geometry="geometry"
            )
